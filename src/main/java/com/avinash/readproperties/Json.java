...
public class Json {
   ...
   /* public static boolean write(String url, String path){
        try{Filer.write(new Gson().toJson(getJsonElement(getUrl(url))), path);return true;}
        catch(Exception exp){System.out.println(exp);return false;}
    }*/
    ...
    public static List<String> getStringArrayList(String json){
        ...
        for(JsonElement element: getJsonArray(json)) list.add(element.toString());
        return list;
    }
    
    public static List<JsonElement> getElementArrayList(String json){
        List<JsonElement> list =new ArrayList<JsonElement>();
        for(JsonElement element: getJsonArray(json)) list.add(element);
        return list;
    }
    ...
    public static URL getUrl(String url){
        try{return new URL(url);}catch(Exception exp){return null;}
    }
}
